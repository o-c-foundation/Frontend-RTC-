...
// Contract address
const CONTRACT_ADDRESS = "0x8ABc0c5f1f85eF0D988f3b7721e0A07381a10abd";
...
const initializeContract = async () => {
  ...
  try {
    ...
    console.log(
      "Attempting to connect to Base mainnet:",
      baseRpcUrl
    );
    ...
    console.log(
      "Successfully connected to Optimism Sepolia. Latest block:",
      blockNumber
    );
...
async function updateTokenInfo(tweet) {
  ...
  try {
    ...
    const tweetUrl = tweetLink.href.replace(/\/photo\/\d+$/, "");
    ...
    if (tokenDataCache.has(tweetUrl)) {
      ...
      try {
        ...
        console.log("Calling contract to get token info", tweetUrl);
...
function handleTokenData(tokenData, tokenInfo, overlay) {
  ...
  const nameValue = tokenInfo.querySelector(
    ".token-info-row:nth-child(1) .token-info-value"
  );
  const tickerValue = tokenInfo.querySelector(
    ".token-info-row:nth-child(2) .token-info-value"
  );
  const contractValue = tokenInfo.querySelector(
    ".token-info-row:nth-child(3) .token-info-value"
  );
  const respectsValue = tokenInfo.querySelector(
    ".token-info-row:nth-child(4) .token-info-value"
  );
  ...
  contractValue.textContent = `${tokenData.tokenAddress.slice(
    0,
    6
...
function showOverlay() {
  ...
  const activityLog =
    document.querySelector(".activity-log") || createActivityLog();
...
function handleKeyPress(event) {
  ...
  if (event.key.toLowerCase() === "f" && hoveredTweet) {
    ...
    if (isTokenExists) {
      ...
      const respectsValue = document.querySelector(
      ...
      try {
        ...
        const replyButton =
          hoveredTweet.querySelector('[data-testid="reply"]') ||
